...
## 0 · SSH auth and config

Every `ssh`, `scp` and `rsync` below relies on the **user's own SSH setup**:
`~/.ssh/config` aliases, agent-forwarded keys, or whatever they already use.
Add `-o BatchMode=yes` on probe/launch calls so an unauthenticated host fails
fast instead of hanging on a password prompt you can never answer.
...
(`Permission denied`, `no supported authentication methods`), do NOT retry in a
loop and do not report the failure yet: tell the user exactly which host needs
sign-in and ask them to authenticate once from their own terminal (run any
trivial command over `ssh <host>`, or set up key-based auth), then retry your
command unchanged.

Two errors no sign-in fixes, so stop and report them instead: a host-key error
(`Host key verification failed` — the user must verify the fingerprint in their
own terminal once) and a network error (unreachable, timed out, unknown host).
Never send a password yourself and never disable host-key checking.

## 1 · Pick the machine

1. `cat .openscience/compute.json` in the workspace if it exists — a plain
   machine inventory that you or the user maintain by hand. It looks like:
   `{"machines":[{"host":"home-3090","label":"8x3090",
     "caps":{"cores":16,"mem_total_bytes":...,"gpus":["RTX 3090",...],"slurm":null}}]}`
2. If the file is missing or has no machines, ask the user for a machine
   (`user@host`, plus GPU/Slurm info if relevant), then offer to create the
   file so next time is one step shorter. Do not guess.
...
4. Confirm it's reachable and check live headroom before launching:
   ...
   On "Permission denied", follow §0 (ask the user to sign in once from their
   terminal), then retry. Never send a password yourself and never disable
   host-key checking.
...
process, so the job survives the connection dropping.
...
4. **Track:**
   ...
   - Finished: `ssh <host> "cat <remote-dir>/exit_code"` — `0` = success, other
     ...
     quick re-run or re-fetch. A run you don't record leaves nothing in the
     workspace's run ledger (`.openscience/remote-runs.jsonl`), so it may as
     well not exist.
...
Recording is not an optional finishing flourish: the remote machine is outside
the workspace, so this call is the ONLY thing that makes the run traceable
later. Do it
...
```bash
python "<skill-dir>/record_run.py" \
...
(`<skill-dir>` = the folder containing this SKILL.md; the harness reports it as
the skill base directory when this skill loads. The helper writes the record to
`.openscience/remote-runs.jsonl` in the workspace.)
