...
```bash
python "<skill-dir>/pdf_extract.py" MANUSCRIPT.pdf
```
...
1. Read `.openscience/provenance.jsonl` in the workspace if present — an
   optional file-version ledger, one JSON record per line:
   `{path, version, ts, tool, content, …}`; `ts` is epoch seconds. Nothing
   writes it automatically here; when it exists it records every file version
   the agent chose to log. Read it directly (`cat .openscience/provenance.jsonl`)
   rather than relying on `ls` (hidden directory). When it is absent — the
   normal case — fall back to file mtimes and say which basis you used.
...
End the reply with exactly one fenced block (keep it as the LAST thing in the
message):
