...
Modal account. Like the HPC/Slurm path, nothing here handles credentials — you
...
Modal must be installed and authenticated on this machine. Verify before writing
code:
...
Modal runs on remote cloud hardware outside the workspace, so recording is the
ONLY thing that makes the run traceable later. Do it after **every** completed
run — including quick re-runs. Skipping it loses the run entirely.
Record it completely (from the workspace root):
