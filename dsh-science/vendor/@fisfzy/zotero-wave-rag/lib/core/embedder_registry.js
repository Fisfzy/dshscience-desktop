...
export function embedderPreset(id) {
    return EMBEDDER_PRESETS.find((p) => p.id === id);
}
/** Resolve a preset into a concrete embedder config, given env secrets. */
export function presetToEmbedder(preset, envKey, envBaseURL) {
    if (preset.kind === 'hash')
        return { embedder: 'hash' };
    if (!preset.model)
        return { embedder: 'api' };
    if (!envKey) {
...
export function embedderCacheId(embedder, model) {
