 ...
 */
import { chunkPaper } from "../ingest/chunker.js";
import { guardField } from "./claim_check.js";
const DEFAULT_OPTS = {
    relatedTopN: 5,
    maxEvidence: 4,
    snippetChars: 320,
};
function graphNeighbors(index, key) {
    const out = [];
...
function collectEvidence(paper, chunkTexts, maxEvidence, snippetChars) {
    const evidence = [];
    for (const a of paper.annotations) {
        const quote = a.quote?.trim();
        if (quote)
            evidence.push(`批注: ${quote.slice(0, snippetChars)}${a.note ? ` — ${a.note.slice(0, 120)}` : ''}`);
...
function extractiveBody(paper, chunks, opts) {
...
async function llmBody(paper, evidence, provider) {
    // baseURL already includes the API prefix; append the path verbatim.
    const url = `${provider.baseURL.replace(/\/+$/, '')}/chat/completions`;
    const system = 'You summarize academic papers. Return ONLY a JSON object with three string fields: method, contribution, experiments. ' +
...
export async function generateDetailCard(paper, index, opts = {}) {
