...
const TAG_WEIGHT = (tagFreq) => 1 / Math.log(2 + tagFreq);
...
export function buildGraph(input) {
    ...
    const allTags = (p) => [...new Set([...p.tags, ...(p.autoTags ?? [])])];
    for (const p of papers) {
        for (const t of allTags(p)) {
            ...
            for (const q of papers) {
                if (q.key <= p.key)
                    continue;
                if (allTags(q).includes(t))
                    add(p.key, q.key, 'tag', w);
            }
        }
    }
    // Author edges.
    ...
    for (const p of papers) {
        ...
        for (const q of papers) {
            ...
            if (sharesTag)
                continue;
            const sharesAuthor = p.creators.some((c) => q.creators.some((d) => `${c.firstName ?? ''} ${d.lastName}`.trim() === `${d.firstName ?? ''} ${d.lastName}`.trim()));
            const sharesCollection = p.collections.some((c) => q.collections.includes(c));
            if (!sharesAuthor && !sharesCollection)
                continue;
            // Skip pairs already linked by k-NN (those are close in vector space).
            if (seen.has(`knn:${p.key}:${q.key}`) || seen.has(`knn:${q.key}:${p.key}`))
                continue;
            const qProfile = meanEmbedding(embeddingsByPaper.get(q.key));
            if (profile && qProfile && cosine(profile, qProfile) >= WORMHOLE_MAX_SIM)
                continue;
