...
export function cacheDir(dataDir) {
    if (process.env.ZWR_CACHE_DIR)
        return process.env.ZWR_CACHE_DIR;
    if (dataDir)
        return join(dataDir, '.zwr-cache');
    return join(tmpdir(), 'zotero-wave-rag-cache');
}
function cachePath(cache, paperKey) {
 ...
 */
export async function fullTextForPdf(paperKey, pdfPath, dataDir) {
...
let _checked;
export function hasPdfTextTool() {
