...
import { resolveConfig, setRuntimeEmbedder, type PluginConfig } from './config.ts'
import { EMBEDDER_PRESETS, embedderPreset } from './embedder_registry.ts'
import type { DetailCard, LibraryIndex, RetrievalHit, RetrievalResult } from './types.ts'
import { buildIndex, loadLibrary } from '../ingest/indexer.ts'
import { createEmbedder, type Embedder } from '../ingest/embedder.ts'
import { denseSearch } from '../retrieval/dense.ts'
import { waveSearch } from '../retrieval/wave.ts'
import { bm25Search, fuseHits, selectSnippets } from '../retrieval/bm25.ts'
import { expandQuery } from '../retrieval/expand.ts'
import { buildQueryPlan, generateQueryPlanWithModel, type QueryPlan } from '../retrieval/query_plan.ts'
import { generateDetailCard } from '../generate/detail_card.ts'
...
export interface CompareResult {
  keys: string[]
  cards: (DetailCard | { key: string; error: string })[]
  sharedTags: string[]
...
export class Engine {
  config: PluginConfig
  private indexPromise: Promise<LibraryIndex> | undefined
  private embedderPromise: Embedder | undefined
  ...
  async status(): Promise<StatusReport> {
    ...
    const notes: string[] = []
    let ok = true
    let index: StatusReport['index'] = { built: false, papers: 0, chunks: 0, edges: 0 }
    let degraded: StatusReport['degraded']
    try {
    ...
    if (!cfg.enableSemantic) semanticReason = '语义通道已通过 ZWR_SEMANTIC=0 关闭（当前为 BM25-only）'
    else if (cfg.embedder === 'hash') semanticReason = 'hash 为词法近似嵌入，非真语义；可切换 API 嵌入（zotero_embedder）'
    if (degraded) notes.push(`⚠ 嵌入降级: ${degraded.from} → ${degraded.to}（${degraded.reason}）；本次索引未写缓存，下次构建将重试 ${degraded.from}`)
    ...
    if (cfg.embedderApi?.model) report.embedderModel = cfg.embedderApi.model
    if (semanticReason) report.semanticReason = semanticReason
  ...
  async search(
    ...
    if (!this.config.enableSemantic && engineKind === 'wave') {
      ...
      const hits: RetrievalHit[] = bm.hits.map((h) => ({
        paperKey: h.paperKey,
        title: h.title,
    ...
    let plan: QueryPlan = buildQueryPlan(query, index)
    ...
    const bmByKey = new Map<string, { title: string; score: number }>()
    ...
    const hits: RetrievalHit[] = fused.map((h) => ({
      paperKey: h.paperKey,
      title: h.title,
      score: h.score,
      reasons: h.reasons,
  ...
  findPaper(index: LibraryIndex, key: string) {
  ...
  async paperDetail(key: string): Promise<DetailCard | { error: string }> {
    this.refresh()
    const index = await this.getIndex()
  ...
  async compare(keys: string[]): Promise<CompareResult> {
    this.refresh()
    const index = await this.getIndex()
    const cards: (DetailCard | { key: string; error: string })[] = []
    const tagSets: string[][] = []
    const creatorSets: string[][] = []
  ...
  currentEmbedderId(): string {
    const cfg = this.config
    if (cfg.embedder === 'hash') return 'hash'
   ...
   */
  setEmbedder(id: string): { ok: boolean; current: string; message: string } {
