...
function dcg(ranks: boolean[]): number {
...
export function evaluate(cases: RankedCase[], topK = 5): Metrics {
  let recallSum = 0
  let mrrSum = 0
  let ndcgSum = 0
  let diversitySum = 0
...
let tagsByKey: Map<string, string[]> = new Map()
export function setTagMap(map: Map<string, string[]>): void {
  tagsByKey = map
}

function pairwiseOverlap(c: RankedCase, top: string[]): number {
