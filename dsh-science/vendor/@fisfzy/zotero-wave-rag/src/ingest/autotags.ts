...
export const AUTO_TAG_LEXICON: string[] = [
...
const METHOD_TYPE_KEYWORDS: Record<Exclude<MethodType, 'mixed' | 'unknown'>, [string, number][]> = {
  experimental: [
 ...
 * ties -> mixed.
 */
export function deriveMethodType(paper: Paper): MethodType {
  const hay = `${paper.title} ${paper.abstract ?? ''}`.toLowerCase()
  let best: MethodType = 'unknown'
  let bestScore = 0
  let tie = false
  for (const [type, kws] of Object.entries(METHOD_TYPE_KEYWORDS) as [Exclude<MethodType, 'mixed' | 'unknown'>, [string, number][]][]) {
    let score = 0
 ...
 */
export function deriveAutoTags(paper: Paper, lexicon: string[] = AUTO_TAG_LEXICON): string[] {
  const hay = `${paper.title} ${paper.abstract ?? ''}`.toLowerCase()
  const found: { term: string; len: number }[] = []
  for (const term of lexicon) {
    if (hay.includes(term.toLowerCase())) found.push({ term, len: term.length })
  }
