...
import type { IndexLevel, PluginConfig } from '../core/config.ts'
import type { Chunk, GraphEdge, LibraryIndex, Paper } from '../core/types.ts'
import { chunkPaper } from './chunker.ts'
import { createEmbedder, type Embedder } from './embedder.ts'
import { embedderCacheId } from '../core/embedder_registry.ts'
import { buildGraph } from './graph.ts'
import { buildBm25Index } from '../retrieval/bm25.ts'
import { AUTO_TAG_LEXICON, deriveAutoTags, deriveMethodType } from './autotags.ts'
import { SAMPLE_PAPERS } from '../zotero/sample.ts'
import { readZoteroLibrary, mapPdfPaths } from '../zotero/db.ts'
import { fullTextForPdf, hasPdfTextTool } from '../zotero/pdf.ts'
...
interface CacheMetaV3 {
  ...
  /** Paper order defines the embedding block layout in the .emb file. */
  perPaper: { key: string; textHash: string; chunks: ChunkMeta[] }[]
}

function cacheFiles(config: PluginConfig): { meta: string; emb: string } {
...
function paperTextHash(paper: Paper, level: IndexLevel): string {
  const full = level === 'fulltext' ? paper.fullText ?? '' : ''
  return fnv1a(`${paper.title}\u0000${paper.abstract ?? ''}\u0000${full}`)
}

function readCacheMeta(config: PluginConfig): CacheMetaV3 | undefined {
  if (!config.dataDir) return undefined
  const { meta, emb } = cacheFiles(config)
  if (!existsSync(meta) || !existsSync(emb)) return undefined
  try {
    const head = JSON.parse(readFileSync(meta, 'utf8')) as CacheMetaV3
    if (!Array.isArray(head.perPaper) || head.perPaper.length === 0) return undefined
    return head
  } catch {
    return undefined
  }
...
function hydrateFromCache(config: PluginConfig, meta: CacheMetaV3, papers: Paper[]): LibraryIndex | undefined {
  if (papers.length !== meta.perPaper.length) return undefined
  for (let i = 0; i < papers.length; i++) {
    if (meta.perPaper[i]!.key !== papers[i]!.key) return undefined
    if (meta.perPaper[i]!.textHash !== paperTextHash(papers[i]!, config.indexLevel)) return undefined
  }
  try {
...
export function loadLibrary(config: PluginConfig): LoadedLibrary {
  if (config.dataDir) {
...
export async function buildIndex(config: PluginConfig, opts: BuildIndexOptions = {}): Promise<LibraryIndex> {
  ...
  if (source === 'zotero' && config.dataDir && hasPdfTextTool()) {
    const pdfs = mapPdfPaths(
      config.zoteroDbPath ?? `${config.dataDir}/zotero.sqlite`,
      config.storageDir ?? `${config.dataDir}/storage`,
    )
  ...
  for (const p of papers) {
    ...
    if (opts.autoTags !== false) {
      ...
      if (auto.length > 0) p.autoTags = auto
    }
  }
  ...
  const changed: { paper: Paper; chunks: Chunk[] }[] = []
  const reused: { paper: Paper; oldEntry: CacheMetaV3['perPaper'][number] | undefined }[] = []
  for (const p of papers) {
  ...
  log(
    `[ingest] incremental: ${reused.length} unchanged (reuse embeddings), ${changed.length} changed (embed ${changed.reduce((s, c) => s + c.chunks.length, 0)} chunks)`,
  )

  // embedder with P1-1 auto-degradation
  let embedder: Embedder = createEmbedder(config.embedder, config.embedderApi)
  let degraded: LibraryIndex['degraded'] | undefined
  const batchSize = embedder.kind === 'api' ? 16 : 64 // P1-1: API batches capped at 16
  const changedWithVecs: { paper: Paper; chunks: Chunk[]; vecs: number[][] }[] = []
  ...
  // 4. graph + BM25
  const edges: GraphEdge[] = buildGraph({ papers, embeddingsByPaper })
