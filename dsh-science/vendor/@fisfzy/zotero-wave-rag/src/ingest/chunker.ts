 ...
 */
function cjkSpacedWithOffsets(text: string, base: number, page?: number): TokenSpan[] {
  let spaced = ''
  const map: number[] = [] // spaced position -> original offset (relative to text)
  let last = 0
  let m: RegExpExecArray | null
  CJK4_RE.lastIndex = 0
  while ((m = CJK4_RE.exec(text)) !== null) {
    spaced += text.slice(last, m.index) + m[0]
    for (let i = last; i < m.index + m[0].length; i++) map.push(i)
    spaced += ' '
  ...
  for (let i = 0; i < spaced.length; i++) {
    ...
    if (/\s/.test(ch)) {
      ...
      if (!cur) curStartPos = i
      cur += ch
    }
  }
  if (cur) spans.push({ token: cur, start: base + map[curStartPos]!, end: base + map[map.length - 1]!, page })
  return spans
}
...
function detectSection(line: string): string | undefined {
  ...
  if (/^(\d+(\.\d+)*\.?)\s+[A-Z]/.test(trimmed)) return trimmed
  if (/^[A-Z][A-Za-z ]{2,40}$/.test(trimmed) && trimmed.length <= 60) return trimmed
  return undefined
}

/** Slice token spans into windows; text = tokens joined with ' ' (unchanged). */
function chunkSpans(spans: TokenSpan[], max: number, overlap: number): TokenSpan[][] {
  if (spans.length <= max) return [spans]
  const out: TokenSpan[][] = []
  for (let i = 0; i < spans.length; i += max - overlap) {
    out.push(spans.slice(i, i + max))
    if (i + max >= spans.length) break
  }
  return out
}
...
export function chunkPaper(paper: Paper): Chunk[] {
  ...
  const pages: { start: number; content: string }[] = []
  {
    let pgStart = 0
  ...
  const sections: { heading?: string; spans: TokenSpan[] }[] = []
  let current: { heading?: string; spans: TokenSpan[] } = { spans: [] }
  ...
  if (current.spans.length > 0) sections.push(current)
