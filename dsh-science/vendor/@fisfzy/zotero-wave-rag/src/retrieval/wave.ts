...
function normalizeMap(map: Map<string, number>): Map<string, number> {
  ...
  const span = max - min
  for (const [k, v] of map) out.set(k, span < EPS ? 0.5 : (v - min) / span)
  return out
}
...
function anchorScore(query: string, titles: string[], authors: string[], tags: string[]): number {
  const q = tokens(query)
  if (q.size === 0) return 0
  const target = tokens([...titles, ...authors, ...tags].join(' '))
  if (target.size === 0) return 0
  let hit = 0
  for (const t of q) if (target.has(t)) hit++
  // Prefer exact title / author matches: a query containing the full title is
  // a hop-0 anchor by construction.
  for (const title of titles) {
    if (query.trim().toLowerCase() === title.toLowerCase()) return 1
  }
...
function pairSim(
  aTags: string[],
  bTags: string[],
  aProfile: number[] | undefined,
  bProfile: number[] | undefined,
): number {
...
function paperProfile(index: LibraryIndex, key: string): number[] | undefined {
  const vecs = index.chunks.filter((c) => c.paperKey === key).map((c) => c.embedding).filter((v): v is number[] => !!v)
  if (vecs.length === 0) return undefined
...
function buildTransition(
  index: LibraryIndex,
  useWormhole: boolean,
): { M: Map<number, Map<number, number>>; byKey: Map<string, number> } {
  const byKey = new Map(index.papers.map((p, i) => [p.key, i]))
  const M = new Map<number, Map<number, number>>()
  const accum = new Map<number, number>() // row sums
  for (const e of index.edges) {
    if (e.kind === 'wormhole' && !useWormhole) continue
  ...
  for (const [from, row] of M) {
    ...
    if (sum > 0) for (const [to, w] of row) row.set(to, w / sum)
  }
  return { M, byKey }
}

export async function waveSearch(
  index: LibraryIndex,
  embedder: Embedder,
  params: WaveHyperParams,
  query: string,
  ...
  for (const h of dense.hits) seedScores.set(h.paperKey, h.score)
  let seedSum = 0
  for (const v of seedScores.values()) seedSum += v
  for (const k of seedScores.keys()) seedScores.set(k, (seedScores.get(k) ?? 0) / (seedSum || 1))
  ...
  for (let hop = 0; hop < params.propagationHops; hop++) {
    ...
    for (const [from, row] of M) {
      const cur = v[from]!
      if (cur === 0) continue
      for (const [to, w] of row) next[to] = (next[to] ?? 0) + cur * w
    }
  ...
  const candidates = new Map<string, { semantic: number; propagation: number; anchor: number }>()
  for (let i = 0; i < n; i++) {
    const p = index.papers[i]!
    if (v[i]! < EPS && (seedScores.get(p.key) ?? 0) < EPS) continue
  ...
  for (const p of index.papers) for (const t of p.tags) {
  ...
  for (const key of candidates.keys()) {
    ...
    rawScores.set(
      key,
      clamp01((alpha * sScore + beta * iScore + gamma * aScore) / (total || 1)),
    )
  }
  ...
  for (const k of keys) profiles.set(k, paperProfile(index, k))
  const reasons = new Map<string, string[]>()
  for (const key of keys) {
    const r: string[] = []
    if ((sem.get(key) ?? 0) > 0.5) r.push(`dense seed ${(seedScores.get(key) ?? 0).toFixed(3)}`)
    if ((prop.get(key) ?? 0) > 0.5) r.push(`propagated ${(prop.get(key) ?? 0).toFixed(3)}`)
    if ((innovation.get(key) ?? 0) > 0) r.push(`Ω innovation +${(innovation.get(key) ?? 0).toFixed(3)}`)
    if ((anch.get(key) ?? 0) > 0.5) r.push(`anchor ${(anch.get(key) ?? 0).toFixed(3)}`)
  ...
  const hits: WaveHits[] = selected.map((key) => {
    const paper = index.papers.find((p) => p.key === key)!
    return {
      paperKey: key,
      title: paper.title,
