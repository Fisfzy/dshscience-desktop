...
const CITATION_RE =
  /(?:^|[\(（]|\s)([A-Z][A-Za-z\-]+)(?:\s+(?:et al\.?|等(?:人)?))?(?:\s+and\s+[A-Z][A-Za-z\-]+)?(?:,|\s)[\(（]?(\d{4})[\)）]?/g

/** Resolve "(Smith et al., 2020)"-style mentions to library paper titles. */
export function parseCitations(query: string, index: LibraryIndex): string[] {
  const found: string[] = []
  ...
  while ((m = CITATION_RE.exec(query)) !== null) {
    ...
    const candidates = index.papers.filter(
      (p) => !seen.has(p.key) && p.creators.some((c) => c.lastName.toLowerCase().startsWith(name)),
    )
...
export function buildQueryPlan(query: string, index: LibraryIndex): QueryPlan {
 ...
 */
export async function generateQueryPlanWithModel(
  query: string,
  index: LibraryIndex,
  llm: ApiProvider,
  ...
  try {
        ...
        messages: [
          {
            role: 'system',
            content:
              'Rewrite the user research query into up to 6 distinct search variants that could find the same literature in different wording (synonyms, abbreviations, English/Chinese). Return ONLY JSON: {"variants":["..."]}.',
          },
    ...
    const variants = Array.isArray(parsed.variants)
      ? parsed.variants.filter((v): v is string => typeof v === 'string' && v.trim().length > 0).slice(0, 6)
      : []
