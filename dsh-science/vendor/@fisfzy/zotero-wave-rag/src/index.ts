...
export function apply(ctx: { tools: { register(tool: unknown): void } }): void {
  ctx.tools.register(
    defineTool({
      name: 'zotero_status',
      description:
        'Report zotero-wave-rag status: data source, index build state, embedder/LLM providers, and wave-retrieval hyper-parameters.',
      parameters: {},
      output: {
        schema: {
          type: 'object',
  ...
  ctx.tools.register(
    defineTool({
      name: 'zotero_search',
      description:
        'Search the Zotero paper library with wave-semantics retrieval (tag-river graph propagation, wormhole jumps, bell-damper diversity, Ω re-rank) and return ranked paper hits with channel scores.',
      parameters: {
        query: {
          type: 'string',
          required: true,
      ...
      presentCall: (args) => ({
        card: 'generic',
        title: `zotero_search: ${String((args as { query?: unknown }).query ?? '')}`,
        kind: 'other',
        rawInput: args,
      }),
    }),
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'Return a structured paper detail card (metadata, method, contribution, experiments, related papers, evidence) for one library paper by its Zotero key.',
      parameters: {
        key: {
          type: 'string',
          required: true,
      ...
      presentCall: (args) => ({
        card: 'generic',
        title: `zotero_paper_detail: ${String((args as { key?: unknown }).key ?? '')}`,
        kind: 'other',
        rawInput: args,
      }),
    }),
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'List or switch the embedding model used by zotero-wave-rag. Presets: hash (free offline), bge-m3, qwen3-embed-8b, qwen3-vl-embed-8b. Switching persists and the next index build uses the new model (index caches are keyed per embedder, so vectors never mix).',
      parameters: {
        action: {
          type: 'string',
          required: true,
      ...
      presentCall: (args) => ({
        card: 'generic',
        title: `zotero_embedder: ${String((args as { action?: unknown }).action ?? '')}`,
        kind: 'other',
        rawInput: args,
      }),
    }),
  ...
  ctx.tools.register(
    defineTool({
      name: 'zotero_compare',
      description:
        'Return side-by-side detail cards for two or more library papers, highlighting shared and differing tags/creators plus wave-graph relations.',
      parameters: {
        keys: {
          type: 'array',
          required: true,
      ...
      presentCall: (args) => ({
        card: 'generic',
        title: `zotero_compare: ${String((args as { keys?: string[] }).keys?.join?.(' ,') ?? '')}`,
        kind: 'other',
        rawInput: args,
      }),
    }),
