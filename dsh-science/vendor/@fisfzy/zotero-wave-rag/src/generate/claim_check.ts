...
import { terms } from '../retrieval/bm25.ts'
...
export function checkClaim(claim: string, evidence: string[]): ClaimCheckResult {
  ...
  for (const t of expSet) if (!baseSet.has(t)) canonical.add(t)
  ...
  for (let i = 0; i < evidence.length; i++) {
    ...
    let baseHit = 0
    for (const t of baseSet) if (evSet.has(t)) baseHit++
    const baseOverlap = baseSet.size > 0 ? baseHit / baseSet.size : 0
    let canonicalOverlap = 0
    if (canonical.size > 0) {
      let cHit = 0
      for (const t of canonical) if (evSet.has(t)) cHit++
 ...
 */
export function guardField(text: string | undefined, evidence: string[]): string | undefined {
  if (!text || evidence.length === 0) return text
  const out: string[] = []
