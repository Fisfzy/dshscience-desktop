...
export function registerMesh(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        ...
        description: 'Seed and generate a mesh on a part (or its assembly instance) in a model. elementFamily: solid (default if the part has cells) or shell (if it only has faces). solid -> C3D8R/C3D4R; shell -> S4R. size: approximate global seed size; if omitted, auto = bounding-box diagonal / 10. Mesh is applied on the assembly instance when the part has an independent instance (Abaqus requirement).',
        parameters: {
        ...
        async execute(args, exec) {
            ...
            if (requested && !['solid', 'shell'].includes(requested))
...
result={"part":p.name,"target":("assembly:"+mesh_target.name) if mesh_on_assembly else p.name,
        "family":family,"size":size,"elements":len(target.elements),"nodes":len(target.nodes)}`, config.timeoutMs, exec.signal);
            return r.value;
        },
