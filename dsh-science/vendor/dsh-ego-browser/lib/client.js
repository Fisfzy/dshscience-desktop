window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var en = {
      title: "ego-browser",
      intro:
        "Agent browser integration. Configure the Chrome/Chromium binary path and cast parameters below.",
      chromePath: "Browser binary path",
      chromePathHint:
        "Path to the Chrome/Chromium/Edge binary. Empty = auto-detect.",
      ...
      githubMirrorHint:
        "Replaces https://github.com for GitHub downloads, for example https://gh-proxy.com/github.com.",
      egoCliArgs: "Extra ego-browser CLI args",
      egoCliArgsHint:
        "Appended to `ego-browser nodejs` argv. Takes effect on the next ego_* call. Blocked: --status/--stop/--open/--spaces/--help (they exit before the heredoc runs). Use --headless via the headless env instead.",
      chromeArgs: "Extra Chrome launch args",
      chromeArgsHint:
        "Appended to the Chrome launch argv. Takes effect on the next browser cold start (the browser is a singleton — run `ego-browser --stop` or restart DSH to relaunch). Blocked: --user-data-dir/--remote-debugging-port/--remote-allow-origins/--headless/--no-startup-window/--proxy-server (use EGO_LINUX_PROXY for the proxy).",
      save: "Save",
      saving: "Saving…",
      discard: "Discard",
      unsaved: "Unsaved",
      ...
      namespaceUnavailable:
        "The ego-browser configuration channel is unavailable. Please retry later.",
      retry: "Retry",
    ...
    var zh = {
      title: "ego-browser",
      intro:
        "Agent 浏览器集成。在下方配置 Chrome/Chromium 浏览器路径及推流参数。",
      chromePath: "浏览器路径",
      ...
      githubMirrorHint:
        "替换 https://github.com，例如 https://gh-proxy.com/github.com。",
      egoCliArgs: "ego-browser CLI 附加参数",
      egoCliArgsHint:
        "追加到 `ego-browser nodejs` argv。下一次 ego_* 工具调用即生效。禁止：--status/--stop/--open/--spaces/--help（会在 heredoc 执行前退出）。--headless 请走 headless 环境变量。",
      chromeArgs: "Chrome 启动附加参数",
      chromeArgsHint:
        "追加到 Chrome 启动 argv。仅在浏览器下次冷启动时生效（浏览器是单例常驻——需运行 `ego-browser --stop` 或重启 DSH 才会重新启动）。禁止：--user-data-dir/--remote-debugging-port/--remote-allow-origins/--headless/--no-startup-window/--proxy-server（代理请用 EGO_LINUX_PROXY）。",
      save: "保存",
      saving: "保存中…",
      discard: "放弃",
      unsaved: "未保存",
        ...
        .then(function (parsed) {
          ...
          self.store.update(function (s) {
            ...
            s.draft = {
              ...
              captureBackend:
                config.captureBackend === "ffmpeg" &&
                !ffmpegStatus.canSelectFfmpeg
                  ? "cdp"
                  : config.captureBackend || "auto",
              ...
              cdpBackstopIntervalMs: String(
                config.cdpBackstopIntervalMs ?? 3e3
              ),
    ...
    EgoBrowserSettingsController.prototype._setFfmpegStatus = function (
      status
    ) {
      var self = this;
      if (!status) return;
      this.store.update(function (s) {
        ...
        if (!status.canSelectFfmpeg && s.draft.captureBackend === "ffmpeg")
          s.draft = Object.assign({}, s.draft, { captureBackend: "cdp" });
      });
    ...
    EgoBrowserSettingsController.prototype._ffmpegRequest = function (
      method,
      body
    ) {
      var self = this;
    ...
    EgoBrowserSettingsController.prototype.installFfmpeg = function () {
      ...
      this._ffmpegRequest("ffmpeg-install", {
        githubMirror: snapshot.draft.githubMirror || "",
      }).then(function () {
    ...
    EgoBrowserSettingsController.prototype._doSave = function () {
      ...
      fetch("/ego/api/set", {
        ...
        .then(function (parsed) {
          ...
          if (!parsed || parsed.ok !== true || !parsed.value) {
            self.store.update(function (s) {
              s.applyState = {
                kind: "error",
                message:
                  parsed && parsed.error ? parsed.error.message : "Save failed",
              };
            });
            return;
          }
          var config = parsed.value.config || {};
          var ffmpegStatus =
            parsed.value.ffmpegStatus || self.store.getSnapshot().ffmpegStatus;
          self.staged.clear();
          self.store.update(function (s) {
...
`;
    ...
    function SettingsField(props) {
      ...
      var inputEl = props.options
        ...
        : h("input", {
            ...
            className:
              "dsh-ego-card__input" +
              (narrow ? " dsh-ego-card__input--narrow" : ""),
      ...
      return h(
        ...
        narrow
          ? h(
              "div",
              { className: "dsh-ego-card__field-row" },
              inputEl,
              unit ? h("span", { className: "dsh-ego-card__unit" }, unit) : null
            )
          : inputEl,
    ...
    function EgoBrowserCard(props) {
      ...
      var header = h(
        ...
        h(
          "span",
          { className: "dsh-ego-card__head-text" },
          h("span", { className: "dsh-ego-card__name" }, t("title")),
          h("span", { className: "dsh-ego-card__desc" }, t("intro"))
        ),
        state.dirty
          ? h("span", { className: "dsh-ego-card__pending" }, t("unsaved"))
          : null,
        h("span", {
          className:
            "dsh-ego-card__chevron" +
            (open ? " dsh-ego-card__chevron--open" : ""),
      ...
      if (!open) body = null;
        ...
        body = h(
          ...
          h(
            "div",
            { className: "dsh-ego-card__footer" },
            h(
              "button",
              {
                type: "button",
        ...
        var ffmpegLabelKey =
          ffmpegStatus.state === "ready"
            ? "ffmpegReady"
            : ffmpegStatus.state === "checking"
            ? "ffmpegChecking"
            : ffmpegStatus.state === "downloading"
            ? "ffmpegDownloading"
            : ffmpegStatus.state === "verifying"
            ? "ffmpegVerifying"
            : ffmpegStatus.state === "extracting"
            ? "ffmpegExtracting"
            : ffmpegStatus.state === "probing"
            ? "ffmpegProbing"
            : ffmpegStatus.state === "unsupported"
            ? "ffmpegUnsupported"
            : ffmpegStatus.state === "failed"
            ? "ffmpegFailed"
            : "ffmpegMissing";
        ...
        var progressText = progress
          ? formatBytes(progress.receivedBytes || 0) +
            (progress.totalBytes
              ...
              : "") +
            (progress.percent !== null && progress.percent !== void 0
              ? " (" + progress.percent + "%)"
              : "")
          : "";
          ...
          h(
            ...
            h(SettingsField, {
              ...
              options: [
                ...
                {
                  value: "ffmpeg",
                  label: ffmpegStatus.canSelectFfmpeg
                    ? "ffmpeg"
                    : t("ffmpegRequired"),
            ...
            h(SettingsField, {
              ...
              placeholder:
                "--disable-features=Translate --window-size=1024,768",
              disabled: busy,
              onEdit: function (v) {
            ...
            h(
              "div",
              { className: "dsh-ego-card__ffmpeg" },
              h(
                "div",
                { className: "dsh-ego-card__ffmpeg-title" },
                t("ffmpegTitle")
              ),
              h(
                "div",
                ...
                t(ffmpegLabelKey) +
                  (ffmpegStatus.reason ? ": " + ffmpegStatus.reason : "") +
              ...
              progress &&
                ...
                ? h(
                    "div",
                    { className: "dsh-ego-card__progress" },
                    h("div", {
              ...
              ffmpegStatus.version
                ? h(
                    "div",
                    { className: "dsh-ego-card__ffmpeg-status" },
                    t("ffmpegVersion") + ": " + ffmpegStatus.version
                  )
                : null,
              h(
                "div",
                { className: "dsh-ego-card__ffmpeg-actions" },
                h(
                  "button",
                  {
                    type: "button",
                ...
                ffmpegStatus.canDownload
                  ? h(
                      "button",
                      {
                        type: "button",
                        className:
                          "dsh-ego-card__btn dsh-ego-card__btn--primary",
                        disabled: ffmpegBusy,
                        onClick: function () {
                          controller.installFfmpeg();
                        },
                      },
                      ffmpegStatus.source === "managed"
                        ? t("ffmpegReinstall")
                        : t("ffmpegInstall")
                    )
                  : null
    ...
    function createKeyboardProxy(send) {
      ...
      input.style.cssText =
        "position:fixed;z-index:2147483647;width:1px;height:1px;opacity:.01;pointer-events:none;resize:none;padding:0;border:0;left:0;top:0;";
      ...
      var releaseAllKeys = function () {
        for (const record of pressed.values())
          send(
            record.targetId,
            "keyUp",
            Object.assign({}, record.payload, { autoRepeat: false })
          );
        pressed.clear();
      };
      ...
      input.addEventListener("beforeinput", function (e) {
        ...
        if (
          e.data &&
          e.data === lastCompositionText &&
          Date.now() - lastCompositionAt < 100
        ) {
          e.preventDefault();
          return;
        }
        if (
          e.data &&
          e.data === lastPasteText &&
          Date.now() - lastPasteAt < 100
        ) {
          e.preventDefault();
          return;
        }
    ...
    function createMsePlayer(video, generation, mime, onFailure) {
      ...
      function pump() {
        if (
          ...
          reading ||
          !reader ||
          queuedBytes >= MAX_QUEUED_VIDEO_BYTES
        )
          return;
        reading = true;
        reader
      ...
      mediaSource.addEventListener("sourceopen", function () {
        ...
        sourceBuffer.addEventListener("updateend", function () {
          if (video.buffered.length) {
            ...
            if (!sourceBuffer.updating && video.buffered.start(0) < end - 3)
              try {
        ...
        fetch("/api/ego/video?generation=" + encodeURIComponent(generation), {
          signal: abort.signal,
        })
          .then(function (res) {
            if (!res.ok || !res.body)
              throw new Error("视频流连接失败 (" + res.status + ")");
    ...
    function mountFloatingWatch(ctx) {
      ...
      const renderHistory = (spaces) => {
        ...
        const list = Array.isArray(spaces)
          ? [...spaces].sort(
              (a, b) => (a.lastActive ?? 0) - (b.lastActive ?? 0)
            )
          : [];
        if (list.length === 0) {
      ...
      const applyCaptureStatus = (status) => {
        ...
        if (
          ...
          status.generation < streamGeneration
        )
          return;
        if (
          status.generation === streamGeneration &&
        ...
        if (changed) {
          stopVideo();
          const current = lastList.find(
            (s) => s.targetId === (selectedTabId || currentActiveId)
          );
      ...
      const syncWatch = (targetId) => {
        ...
        const request = requestWatch(
          watchStarted ? WATCH_SWITCH_ROUTE : WATCH_START_ROUTE,
          {
        ...
        request
          ...
          .catch(() => {});
        if (!watchRenewTimer)
          watchRenewTimer = window.setInterval(() => {
      ...
      const sendInput = (targetId, type, params) => {
        if (
          !!!targetId ||
          (type !== "mouseReleased" && type !== "keyUp" && !effectiveVisible())
        )
          return;
      ...
      const keyboardProxy = createKeyboardProxy((targetId, type, params) =>
        sendInput(targetId, type, params)
      );
      /**
      ...
      const makeZoomImage = (urlEl, tagName = "img") => {
        ...
        const resetView = () => {
          ...
          apply();
          showHint(
            "已复位 · 滚轮滚动页面 · Ctrl+滚轮缩放 · Ctrl+拖动平移 · 点按/拖动操作浏览器 · 双击复位"
          );
        };
        img.title =
          "滚轮滚动页面 · Ctrl+滚轮缩放 · Ctrl+拖动平移 · 点按/拖动=操作浏览器 · 双击复位";
        img.addEventListener(
          ...
          (e) => {
            ...
            if (e.ctrlKey || e.metaKey) {
              ...
              const next = Math.min(
                8,
                Math.max(1, zoomState.scale * (e.deltaY < 0 ? 1.15 : 1 / 1.15))
              );
              if (next <= 1) {
                resetView();
                return;
              ...
              zoomState.tx =
                mx - (mx - zoomState.tx) * (next / zoomState.scale);
              zoomState.ty =
                my - (my - zoomState.ty) * (next / zoomState.scale);
            ...
            if (p && liveImgTargetId)
              sendInput(liveImgTargetId, "mouseWheel", {
                x: p.x,
                y: p.y,
                deltaX: e.deltaX || 0,
        ...
        img.addEventListener("pointermove", (e) => {
          ...
          if (!browserDrag) {
            const p = browserXY(e);
            if (p)
              sendInput(liveImgTargetId, "mouseMoved", {
                x: p.x,
                y: p.y,
                buttons: 0,
              });
        ...
        const stopDrag = (e) => {
          ...
          if (browserDrag) {
            ...
            if (lastDragPos && dragTargetId)
              sendInput(dragTargetId, "mouseReleased", {
                x: lastDragPos.x,
                y: lastDragPos.y,
                button: "left",
                buttons: 0,
      ...
      const renderTabs = (spaces) => {
        ...
        for (const s of list) {
          ...
          if (
            s.targetId === selectedTabId ||
            (selectedTabId === null && s.targetId === currentActiveId)
          )
          ...
          tab.addEventListener("click", () => {
            ...
            [...tabsEl.querySelectorAll(".dsh-ego-tab")].forEach((t) =>
              t.classList.toggle("active", t.__tid === s.targetId)
            );
          });
      ...
      const renderSingleView = (s) => {
        ...
        badge.innerHTML = pinned
          ...
          : `<span class="dsh-ego-state-dot${
              fab.classList.contains("dsh-ego-busy") ? " busy" : ""
            }"></span> 正在实时浏览`;
        ...
        if (captureBackend === "ffmpeg" || cached) {
          const img = makeZoomImage(
            u,
            captureBackend === "ffmpeg" ? "video" : "img"
          );
          if (captureBackend === "ffmpeg" && streamState === "streaming") {
            stopVideo();
            videoCleanup = createMsePlayer(
              img,
              streamGeneration,
              streamMime,
              (message) => {}
      ...
      const renderSpaces = (spaces) => {
        ...
        for (const s of lastList) {
          ...
          pageMeta.set(s.targetId, {
            ...
            title: s.title,
            targetId: s.targetId,
            ...(Number.isFinite(s.viewportW)
              ? { vw: s.viewportW }
              : prev.vw !== void 0
              ? { vw: prev.vw }
              : {}),
            ...(Number.isFinite(s.viewportH)
              ? { vh: s.viewportH }
              : prev.vh !== void 0
              ? { vh: prev.vh }
              : {}),
        ...
        for (const id of [...pageMeta.keys()])
          if (!liveIds.has(id)) pageMeta.delete(id);
        for (const id of [...frameCache.keys()])
          if (!liveIds.has(id)) frameCache.delete(id);
        const sorted = [...lastList].sort(
          (a, b) => (b.lastActive ?? 0) - (a.lastActive ?? 0)
        );
        const activeMarked =
          lastList.find((s) => s.active === true) || sorted[0] || null;
        if (activeMarked) {
        ...
        const busy = lastList.some(
          (s) => Date.now() - (s.lastActive || 0) <= ACTIVE_WINDOW_MS
        );
        ...
        const sel =
          selectedTabId !== null
            ? lastList.find((x) => x.targetId === selectedTabId)
            : null;
      ...
      const openStream = () => {
        if (disposed) return;
        fetch(WATCH_STATUS_ROUTE, { cache: "no-store" })
      ...
      const snapPanelToFab = () => {
        ...
        const ph =
          panel.offsetHeight > 0
            ? panel.offsetHeight
            : Math.min(Math.round(vh * 0.7), 520);
          ...
          if (dragged) {
            ...
            try {
              localStorage.setItem(
                which === "fab" ? DRAG_KEY : DRAG_PANEL_KEY,
      ...
      const maybeShowLoginGuide = () => {
        ...
        const show =
          lastList.some((s) => s.url && !s.url.startsWith("about:")) &&
      ...
      const maybeShowCaptchaGuide = () => {
        ...
        if (show && captchaKindEl)
          captchaKindEl.textContent = hit.humanCheck.kind || "captcha";
      };
      loginBtn.addEventListener("click", () => {
        (async () => {
          ...
          try {
            ...
            if (j && j.ok)
              loginNote.textContent = `已保存 ${j.total ?? ""} 条会话`;
...
`;
    function LivePreviewController(ctx) {
      ...
      var self = this;
      this.keyboardProxy = createKeyboardProxy(function (
        targetId,
        type,
        params
      ) {
    ...
    LivePreviewController.prototype._recompute = function () {
      ...
      if (this.pinned) {
        ...
        if (currentSpace && !currentSpace.thumbnail) {
          ...
          if (cached)
            currentSpace = Object.assign({}, currentSpace, {
              thumbnail: cached,
            });
        }
      } else {
        ...
        if (!activeMarked && this.lastList.length > 0)
          activeMarked = this.lastList.slice().sort(function (a, b) {
      ...
      var captchaKind =
        captchaHit && !this.dismissedGuides.captcha
          ? captchaHit.humanCheck.kind || "captcha"
          : null;
      this.store.update(function (s) {
    ...
    LivePreviewController.prototype.dispose = function () {
      ...
      if (this.reconnectFallbackTimer)
        window.clearTimeout(this.reconnectFallbackTimer);
    ...
    LivePreviewController.prototype._applyCaptureStatus = function (status) {
      ...
      if (
        Number.isFinite(status.generation) &&
        status.generation < this.streamGeneration
      )
        return;
      if (
        status.generation === this.streamGeneration &&
    ...
    LivePreviewController.prototype._syncWatch = function (targetId) {
      ...
      if (!this.watchRenewTimer)
        this.watchRenewTimer = window.setInterval(function () {
    ...
    LivePreviewController.prototype._stopWatch = function (immediate) {
      ...
      var stop = function () {
        ...
        postJson(WATCH_STOP_ROUTE, { clientId: self.clientId }).catch(
          function () {}
        );
      };
      if (immediate) stop();
    ...
    LivePreviewController.prototype.setLiveVideo = function (video, targetId) {
      ...
      if (
        ...
        return;
      var self = this;
      this.videoCleanup = createMsePlayer(
        video,
    ...
    LivePreviewController.prototype.applyFrame = function (
      targetId,
      dataUrl,
      vw,
      vh
      ...
      if (
        ...
        if (now - this.lastFrameAt >= FRAME_FOLLOW_MIN_MS) {
          ...
          this.followTimer = window.setTimeout(function () {
            if (self3.disposed || self3.pinned || self3.selectedTabId !== null)
              return;
    ...
    LivePreviewController.prototype.sendInput = function (
      targetId,
      type,
      params
    ) {
      if (
        !!!targetId ||
        this.disposed ||
        (type !== "mouseReleased" && type !== "keyUp" && !this.visible)
      )
        return;
      var self = this;
      fetch(INPUT_ROUTE, {
    ...
    LivePreviewController.prototype.handleWheel = function (e) {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        var next = Math.min(
          8,
          Math.max(1, this.zoomState.scale * (e.deltaY < 0 ? 1.15 : 1 / 1.15))
        );
        if (next <= 1) {
          this.resetZoom();
          return;
        ...
        this.zoomState.tx =
          mx - (mx - this.zoomState.tx) * (next / this.zoomState.scale);
        this.zoomState.ty =
          my - (my - this.zoomState.ty) * (next / this.zoomState.scale);
    ...
    LivePreviewController.prototype.handlePointerMove = function (e) {
      if (!this._pointerState) {
        var p = this.browserXY(e);
        if (p)
          this.sendInput(this.liveImgTargetId, "mouseMoved", {
            x: p.x,
            y: p.y,
            buttons: 0,
          });
      ...
      if (this._pointerState.viewPanning) {
        this.zoomState.tx =
          this._pointerState.stx + (e.clientX - this._pointerState.sx);
        this.zoomState.ty =
          this._pointerState.sty + (e.clientY - this._pointerState.sy);
        this._applyZoom();
        return;
      }
    ...
    function EgoBrowserTab(props) {
      ...
      if (controllerRef.current === null)
        controllerRef.current = new LivePreviewController(ctx);
      ...
      React.useEffect(
        function () {
          ...
          if (videoRef.current && state.currentTargetId != null)
            controller.setLiveVideo(videoRef.current, state.currentTargetId);
        },
        [
      ...
      var header = h(
        "div",
        { className: "dsh-ego-side-head" },
        h(
          "span",
          { className: "dsh-ego-side-title" },
          h("span", { dangerouslySetInnerHTML: { __html: ICON_GLOBE } }),
          h(
            "span",
            { style: { marginLeft: "5px" } },
            state.busy ? "Agent 浏览器 · 实时" : "Agent 浏览器"
          )
        ),
        h(
          "button",
        ...
        h(
          ...
          {
            className:
              "dsh-ego-side-iconbtn" + (state.historyOpen ? "" : " off"),
      ...
      if (state.historyOpen) {
        ...
        return h(
          "div",
          { className: "dsh-ego-side-root" },
          header,
          h(
            "div",
            { className: "dsh-ego-side-history" },
            h(
              "div",
              { className: "dsh-ego-side-historyhead" },
              h("span", { dangerouslySetInnerHTML: { __html: ICON_CLOCK } }),
              " 历史浏览轨迹"
            ),
            h(
              "div",
              { className: "dsh-ego-side-historylist" },
              sorted.length === 0
                ...
                : sorted.map(function (s) {
                    return h(
                      "div",
                      {
                        ...
                        className:
                          "dsh-ego-side-hitem" +
                          (s.targetId === state.currentTargetId
                            ? " active"
                            : ""),
                        onClick: function () {
                          controller.pinTo(s);
                      ...
                      h(
                        "div",
                        { className: "dsh-ego-side-hinfo" },
                        h(
                          "div",
                          { className: "dsh-ego-side-htitle" },
                          s.title || s.url || "新标签页"
                        ),
                        h(
                          "div",
                          { className: "dsh-ego-side-hurl" },
                          s.url || "(about:blank)"
                        ),
                        s.targetId === state.currentTargetId
                          ? h(
                              "div",
                              { className: "dsh-ego-side-hactive" },
                              "● 当前"
                            )
                          : null
                      )
      ...
      var tabsEl =
        state.spaces.length > 0
          ? h(
              "div",
              { className: "dsh-ego-side-tabs" },
              state.spaces.map(function (s) {
                var isActive =
                  s.targetId === state.selectedTabId ||
      ...
      if (!currentSpace)
        body = h(
          "div",
          { className: "dsh-ego-side-body" },
          h(
            "div",
            { className: "dsh-ego-side-empty" },
            h("div", null, "暂无活跃浏览器页"),
            h(
              "div",
              { style: { fontSize: "11px" } },
              "当 agent 开始用 ego_* 操作网页时，这里会实时显示"
            )
          )
        );
      else {
        var liveImg =
          state.backend === "ffmpeg"
            ...
            : h(
                "div",
                { className: "dsh-ego-side-liveurl" },
                state.streamMessage ||
                  "（暂无截图 — about:blank 或浏览器未渲染）"
              );
        body = h(
          "div",
          { className: "dsh-ego-side-body" },
          h(
            "div",
            { className: "dsh-ego-side-liveview" },
            h(
              "div",
              ...
              h("span", {
                className:
                  "dsh-ego-side-state-dot" +
                  (state.pinned ? " pin" : state.busy ? " busy" : ""),
              }),
              h(
                "span",
                { style: { flex: 1 } },
                (state.backend === "ffmpeg" ? "FFmpeg · H.264" : "CDP") +
                  " · " +
                  (state.streamState === "failed"
                    ? state.streamMessage || "失败"
                    : state.streamState)
              ),
              state.pinned
                ? h(
            ...
            h(
              "div",
              { className: "dsh-ego-side-livetitle" },
              currentSpace.title || currentSpace.url || "(新标签页)"
            ),
            h(
              "div",
              {
                className:
                  "dsh-ego-side-liveurl" +
                  (state.zoomHint ? " dsh-ego-side-hint" : ""),
              },
              state.zoomHint || currentSpace.url || ""
            )
          )
        );
      }
    ...
    function EgoLoginGuide(props) {
      ...
      return h(
        "div",
        { className: "dsh-ego-side-login" },
        h(
          "span",
          { className: "dsh-ego-side-login-txt" },
          "需要账号登录时，请到桌面上那个 ",
    ...
    function EgoCaptchaGuide(props) {
      ...
      return h(
        "div",
        { className: "dsh-ego-side-captcha" },
        h(
          "span",
          { className: "dsh-ego-side-captcha-txt" },
          h("b", null, "⚠️ 检测到人机验证"),
          " — 请在桌面那个 ",
          h("b", null, "「ego lite — agent」"),
      ...
      var disposeTab = betterSidebar.registerTab({
        ...
        urlTarget: function (url) {
          return (
            ...
            !/\.(pdf|txt|md|docx?|xlsx?|pptx?)$/i.test(url.pathname)
          );
        },
