...
> (`dsh --profile danus-verifier`), cold-started per verification. Wherever this
> contract names a skill as `$<name>` (e.g. `$verify-sequential-statements`), load
> it with the `skill` tool (`skill(name: "<name>")`); the verify skills live in
> `.agents/skills` under your working directory. "Codex's built-in web search" is
> your `web_search` tool. Everything else is verbatim Danus.
