...
export const DEFAULT_EXECUTOR = Object.freeze({
    provider: 'deepseek-official',
...
function resolvePhase(base, override) {
    ...
    if (override.provider !== undefined)
        phase.provider = override.provider;
    if (override.model !== undefined)
        phase.model = override.model;
    if (override.reasoningEffort !== undefined)
        phase.reasoningEffort = brandReasoningEffort(override.reasoningEffort);
    return phase;
}
/**
 ...
 */
export function proposePhaseModel(config, phase) {
    const proposal = { ...config };
    if (phase.provider !== undefined)
        proposal.provider = phase.provider;
    if (phase.model !== undefined)
        proposal.model = phase.model;
    if (phase.reasoningEffort !== undefined)
        proposal.reasoningEffort = phase.reasoningEffort;
    return proposal;
}
/**
 ...
 */
export function phaseFor(session, routing) {
